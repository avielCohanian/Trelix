<template>
    ...
    >
        <div
            v-if="
                card.style && card.style.isFull === false && card.style.bgColor
            "
            class="cover-Half"
            :style="bgColorHalf"
        ></div>
        ...
        <section
            ...
            :class="{
                'img-cover':
                    card.style && card.style.isFull && card.style.bgUrl,
            }"
        >
            ...
            <div class="icons" v-if="isBadgetsExist">
                ...
                <span v-if="card.comments && card.comments.length > 0">
                    <span class="el-icon-chat-round icon"></span
                    >{{ card.comment }}</span
                >
...
export default {
    ...
    computed: {
        isBadgetsExist() {
            ...
            return Object.keys(this.card).some((_key) =>
                badgets.includes(_key)
            );
        },
        bgColorHalf() {
        ...
        bgUrlHalf() {
            if (this.card.style.bgUrl) return this.card.style.bgUrl;
        },
        bgColorFull() {
        ...
        doneTodosAmount() {
            ...
            this.card.checklists.forEach((checklist) => {
                if (checklist.todos && checklist.todos.length) {
                    doneTodos += checklist.todos.filter(
                        (todo) => todo.isDone
                    ).length;
                }
            });
